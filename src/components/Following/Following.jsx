...
const Following = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-gray-100 pr-20 pl-20 pt-10 pb-10">
        ...
        <input onChange={handleSearchUser} placeholder="Search User" className="w-full h-[80px] bg-white rounded-3xl pl-10 text-2xl transition-all duration-400 outline-transparent focus:outline-[#e7acd9]" />
        </div>
      ...
      <div className="w-full grid grid-cols-12 pt-10 gap-5">
        ...
        <p className="w-full col-span-12 mb-5 ml-5 text-[40px]">Following</p>
        <p className="text-[30px]">{user?.following.totalCount}</p>
        </div>
        {
        followersUser?.map((user) => (
          ...
          />
           <p className="text-gray-600 group-hover/item:text-black group-hover/item:font-bold text-center text-md mt-4">{user.login}</p>
          </div>
        ))}
      </div> 
    </div> 
